...
\subsubsection{Campo istantaneo di velocità}
Estraendo il campo di velocità in un determinato istante, si ottiene il campo istantaneo di velocità:
\begin{figure}[H]
    \centering
...
\subsubsection{Campo medio di velocità}
...
\noindent Si osservano dei picchi nella trasformata di Fourier in corrispondenza della frequenza di sfilamento dei vortici. I risultati della FFT cambiano a seconda del punto analizzato nello spazio, ma la frequenza che caratterizza lo sfilamento vorticoso rimane la stessa.\\\\
...
\noindent Il flusso che si sviluppa non risulta essere quello canonico 2D per via della sua dimensione finita che appoggia sulla parete inferiore della camera di prova. Inoltre, il piano di misura è a un diametro di distanza dal fondo, dove si sviluppa il vortice a ferro di cavallo (horseshoe vortex) e sono presenti effetti tridimensionali dovuti all'induzione del vortice. Per questi motivi i casi esaminati differiscono dai flussi canonici bidimensionali, pertanto, l'andamento Reynolds-Strouhal si discosta leggermente da quello studiato per il cilindro 2D utilizzando l'anemometria a filo caldo.

\subsubsection{Conservazione dell'energia (Teorema di Parseval)}
...
\subsubsection{Sforzi di Reynolds}
...
\noindent Gli sforzi di Reynolds forniscono una visione dettagliata delle interazioni turbolente nella scia del cilindro. La presenza di elevate fluttuazioni turbolente nelle regioni di separazione e ricircolo evidenzia l'importanza dei fenomeni turbolenti nel determinare il comportamento globale del flusso.